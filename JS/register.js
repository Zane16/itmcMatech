...
// Fetch the role from the URL parameters
const urlParams = new URLSearchParams(window.location.search);
let role = urlParams.get("role");

// Fallback if role is not found in the URL
role = role || "Client";  // Default to "Client" if no role is found in the URL
  ...
  createUserWithEmailAndPassword(auth, email, password)
    .then((userCredential) => {
      ...
      const userData = {
        email: email,
        username: username,
        role: role, // Store the correct role here
      };
